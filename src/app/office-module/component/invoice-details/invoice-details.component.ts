...
export class InvoiceDetailsComponent implements OnInit {
  ...
  Invoice_path='';payname:any;payamt:any;paymodesist=[];QUANTITY:any;PRICE:any;tot:any;podistrictid:any;
  ADV_PER:any;      DELIVERY_PER:any;      INSTALL_PER:any;      FINAL_PER:any;TOTAL_PER:any;
  ADV_PER_amt:any;      DELIVERY_PER_amt:any;      INSTALL_PER_amt:any;      FINAL_PER_amt:any;TOTAL_PER_amt:any;ADV_PER_AMOUNT:any;
balqty:any;balamt:any;
Comp_ids:any;
units:any;amount:any;
  ADV_STATUS:any;   
  DELIVERY_STATUS:any; 
  INSTALL_STATUS:any; 
  FINAL_STATUS:any; 
  ...
  SLNO:any;
  PAYMODEID1 = false; PAYMODEID2 = true; PAYMODEID3 = false;   PAYMODEID:any; 
  ...
  try {
   // this.toast.info(this.SLNO);
    ...
    if (res.success) {debugger;
      ...
      this.podistrictid=res.result[0].DIST_ID;

      this.balqty=res.result[0].BLC_QTY;
      this.balamt=res.result[0].BLC_AMOUNT;
  ...
  //   this.toast.info(id +"---"+ qty +"---" +amt+"---" +totals);
     ...
     if(element!=null){     if(element.checked ===true ){
        ...
        if(element2.checked ===true ){
           ...
           this.DELIVERY_PER_amt= element1.value;
           
           this.PAYMODEID2=true; 
          //  if(this.Comp_ids===17)
          //    {
          //     this.PAYMODEID2=false;
          //    }
...
debugger;
if(this.ADV_PER_AMOUNT===undefined ) {this.toast.warning('Amount not entered');return;}
if ( this.ADV_PER_AMOUNT==='0') {this.toast.warning('Amount not entered');return;}
if (parseInt(this.ADV_PER_AMOUNT) <=0) {this.toast.warning('Amount not entered');return;}
if(this.ADV_PER===undefined || this.ADV_PER=="0") {this.toast.warning('Qty not entered');return;}
if(parseInt(this.ADV_PER)>parseInt(this.balqty)) {this.toast.warning('You have Enter more than Balance Qty');return;}

 
 if(this.STAMPRECEIPTPATH===undefined ||this.STAMPRECEIPTPATH===''|| this.STAMPRECEIPTPATH===null ) {this.toast.warning('Stamp Receipt (PDF)');return;}

 if(parseInt(this.ADV_PER_AMOUNT)>parseInt(this.ADV_PER_AMOUNT)) {this.toast.warning('You have Enter more than Balance Qty');return;}
 ...
 debugger;
      ...
      ADV_PER_AMOUNT:this.ADV_PER_AMOUNT,//ADV_PER_amt
  ...
  async POBYPONUMBER(): Promise<void> {
    try {   debugger;
      ...
      if (res.success) {  debugger;
        ...
        this.Comp_ids=res.result[0].COMPID;
        if(this.Comp_ids===17)
          {
           this.PAYMODEID2=false;
          }
          else{this.PAYMODEID2=true;

          }
      //   for(let j=0;j<res.result.length;j++){ 
      //     this.amt= (( res.result[j].PRICES)); 
      //   this.totalamt=parseInt(this.totalamt)+ parseInt(this.amt);
      //   }

        this.ADV_PER=res.result[0].units;
       this.ADV_PER_AMOUNT=res.result[0].amount;
