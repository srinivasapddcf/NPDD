...
@Component({
  selector: 'app-po-distrcit-qty-changes',
  templateUrl: './po-distrcit-qty-changes.component.html',
  styleUrls: ['./po-distrcit-qty-changes.component.css'] 
})
export class PoDistrcitQtyChangesComponent implements OnInit {
  ...
  async btnSubmitDetails():Promise<void>{
    ...
    if(parseFloat(this.Updateqty)>parseFloat(this.GridDataold[0].PO_BALANCE_QUANTITY) )
      {
        this.toast.warning('Please Enter Lessthan PO Quntity ');
        return;
      }
    if(parseFloat( this.Updateamt) >  parseFloat( this.GridDataold[0].PO_BALANCE_AMOUNT) )
      {
        this.toast.warning('Please Enter Lessthan   PO Amount');
        return;
      }
    ...
    //     return;
    //   }
      if(parseFloat(this.Updateqty)<=0   ||  parseFloat(this.Updateamt)<=0 )
        {
          this.toast.warning('Please Enter  Morethan 0 Quntity and Amount');
          return;
        }
