import { Component, OnInit, OnDestroy, AfterViewInit, ViewChild, EventEmitter, Output } from '@angular/core';
...
//this.balqty=  parseFloat(this.purchasedetailsList[i].UNITS.toFixed(2))- parseFloat(this.reportTotal.INVOICE_QUANTITY.toFixed(2))
                        ;
    ...
    distid: any;distname:any;
  btnGetDetails(obj:any,DIST:any): void {
this.distid=0;debugger;
    if (DIST == "1")
         {this.distid=obj.DISTRICTID;this.distname=obj.DISTRICTNAME;

         } else this.distname="ALL DISTRICTS";//else this.distid=obj.DISTRICTID;
    const requestData = {
    ...
    const encryptedString = this.utils.encrypt(JSON.stringify(requestData));
      //  this.onDistrictChange.emit(encryptedString);  poinvoicedetailsreportComponent   MechanicalModule/ PoBasedInvoiceDetailsReport
if(this.session.desigId=="50")
    this.router.navigate(['/MechanicalModule/PoBasedInvoiceDetailsReport'],{queryParams: {  request: encryptedString } });
else this.router.navigate(['/officeModule/PoBasedInvoiceDetailsReport'],{queryParams: {  request: encryptedString } });
}
